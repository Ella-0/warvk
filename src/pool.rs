use nix::{
    ...
    sys::{
        mman, signal,
        signal::{SigAction, SigHandler, Signal},
    },
    unistd,
};
use std::{
...
impl Pool {
    pub fn new(fd: RawFd, size: usize) -> Result<Pool, ()> {
        ...
        println!("Creating new shm pool, {} {}", fd, size);
        Ok(Pool {
    ...
    pub fn resize(&self, newsize: i32) -> Result<(), ResizeError> {
        ...
        println!("Resizing shm pool {} {} -> {} ", self.fd, oldsize, newsize);
        guard.remap(newsize as usize).map_err(|()| {
            println!(
                "SHM pool resize failed, {} {} -> {}",
                self.fd, oldsize, newsize
            );
    ...
    pub fn with_data_slice<T, F: FnOnce(&[u8]) -> T>(&self, f: F) -> Result<T, ()> {
        ...
        println!("Buffer access on shm pool, fd {}", self.fd);
        ...
        SIGBUS_GUARD.with(|guard| {
            ...
            if triggered {
                println!("SIGBUS caught on access on shm pool fd {}", self.fd);
                Err(())
            } else {
                Ok(t)
            }
...
impl Drop for Pool {
    fn drop(&mut self) {
        println!("Deleting SHM pool fd {}", self.fd);
